...
namespace details
{
    ...
    size_t str_size(const CharT* const &s)
    {
    ...
    size_t str_size(const std::string& s){
        return s.size();
    }

    template<typename = char>
    size_t str_size(const std::string_view& s) {
        return s.size();
    }
}
...
std::string concat(ArgsT&&... args)
{
    ...
    template_for_each([&totalSize](auto&& arg) {
            ...
            totalSize += str_size(std::forward<ArgT>(arg));
...
std::string format(const std::string_view& fmt, ArgsT&&... args)
{
    ...
    foreach_marker(fmt, [&total_length, &args...](auto&& s, int arg_id) {
        ...
        if (arg_id >= 0)
        {
            total_length += str_size(nth(arg_id, std::forward<ArgsT>(args)...));
        }
    });
    std::string result;
...
// overriding new/delete to verify no more than one allocation per `concat` is done
#include <cstdlib>
...
int main()
{
...

    {
        ...
        const char* bang = "!!!!";
        const std::string hello = "Hello"s;
        auto s = format("- %0, %1 %2%3...", hello, "dear", "world"sv, bang);
